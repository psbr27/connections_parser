...
def snmp_handler_response(new_sock, e, lock):
    global create_flag
    index_counter = 0
    while True:
        ...
        if data:
            ...
            log.info("[%s] Heart beat Rx" %(esc_id))
...
def rest_connections(lock):
    ...
    while True:
        ...
        log.info(connection)
        try:
            t4 = threading.Thread(name='rest_receiver', target=snmp_handler_response, args=(connection, e, lock))
            t4.daemon = True
            t4.start()
